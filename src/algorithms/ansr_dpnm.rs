...
/// AnsrDpnm — Adaptive Neighbourhood Search with Restarts V2
///
...
pub fn new_ansr_dpnm(params: &BTreeMap<String, f32>) -> AnsrDpnm {
    AnsrDpnm {
...
impl Optimizer for AnsrDpnm {
        ...
        F: Fn(&[Vec8]) -> f32 + Sync,
    {
        ...
        for epoch in 0..max_epoch {
            ...
            let cosine_t = 0.5 * (1.0 + f32::cos(std::f32::consts::PI * t));
            let effective_sigma = sigma * cosine_t;
            let neighbour_scale = effective_sigma * (1.0 + self.neighbour_multiplier * 2.0 * cosine_t);
            for p in 0..popsize {
                ...
                for d in 0..params {
                    if random.sample(&mut rng) <= self_instead_neighbour {
                        ...
                        current_positions[p][d] = wrap_to_unit_cube(
                            best_positions[r][d]
                                + normal.sample(&mut rng) * neighbour_scale
...
mod tests {
    ...
    fn run_ansr_dpnm(ansr: &AnsrDpnm, maxiter: u64) -> OptimizerResult {
    ...
    fn test_popsize_2_runs() {
        let ansr = AnsrDpnm { popsize: 2, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 10_000);
        assert!(result.f_x.is_finite());
    }

    #[test]
    fn test_popsize_large_converges() {
        let ansr = AnsrDpnm { popsize: 64, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 100_000);
    ...
    fn test_restart_tolerance_zero_no_restart() {
        let ansr = AnsrDpnm { restart_tolerance: 0.0, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 50_000);
        assert!(result.f_x.is_finite());
    }

    #[test]
    fn test_restart_tolerance_positive_converges() {
        let result = run_ansr_dpnm(&default_ansr_dpnm(), 100_000);
        assert!(result.f_x <= 0.01, "f_x={} > 0.01", result.f_x);
    }
    ...
    fn test_sigma_positive_converges() {
        let ansr = AnsrDpnm { sigma: 0.3, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 100_000);
        assert!(result.f_x <= 0.01, "f_x={} > 0.01", result.f_x);
    }
    ...
    fn test_self_instead_neighbour_zero_social_only() {
        let ansr = AnsrDpnm { self_instead_neighbour: 0.0, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 100_000);
        assert!(result.f_x.is_finite());
    }
    ...
    fn test_self_instead_neighbour_one_individual_only() {
        let ansr = AnsrDpnm { self_instead_neighbour: 1.0, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 100_000);
        assert!(result.f_x.is_finite());
    }
    ...
    fn test_self_instead_neighbour_mid_converges() {
        let ansr = AnsrDpnm { self_instead_neighbour: 0.5, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 100_000);
        assert!(result.f_x <= 0.01, "f_x={} > 0.01", result.f_x);
    }
    ...
    fn test_output_within_bounds() {
        ...
        let ansr = AnsrDpnm { sigma: 0.5, ..default_ansr_dpnm() };
        let func = broadcast_simd(sphere);
        let early_stop = EarlyStopCallback::new(&func, 0.01);
    ...
    fn test_nfev_upper_bound() {
        let popsize = 8;
        let maxiter = 1000;
        let ansr = default_ansr_dpnm();
        let result = run_ansr_dpnm(&ansr, maxiter);
    ...
    fn test_nfev_at_least_one_epoch() {
        let ansr = AnsrDpnm { popsize: 4, ..default_ansr_dpnm() };
        let result = run_ansr_dpnm(&ansr, 100);
        assert!(result.nfev >= 4, "nfev={} < popsize=4", result.nfev);
    }

    #[test]
    fn test_new_ansr_dpnm_from_params() {
